...
Additional headless runtime evidence (2026-09-05): `LUNE_BIN=/tmp/obby-tools/lune ROJO_BIN=/tmp/obby-tools/rojo ./scripts/headless.sh` passed. It built the production place, loaded the actual generated world through Lune's Roblox Instance adapter, and exercised the production validator against removed-support and conflicting-controller mutations. It simulated 1, 2 and 8 players through profile loading, delayed CharacterAdded, initial state, checkpoint traversal and dedupe, fall/reset/help recovery, personal key claims, personal finale delivery, rebuild retention, respawn hooks, replay start, and leave cleanup. Output: `HEADLESS lifecycle: 1 ... passed`, `2 ... passed`, `8 ... passed`, `HEADLESS elapsed 27.496s`. This is useful server lifecycle evidence, but it is not Roblox Studio physics, replication, rendering, input, device, network or human evidence.
