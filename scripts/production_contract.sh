...
grep -q 'math.abs(value) < math.huge' "$ROOT_DIR/src/shared/Util/ProgressionRules.lua" || fail "non-finite progression values are not rejected"
...
grep -q 'elapsed >= math.max(0, minimumTimeTrialSeconds)' "$ROOT_DIR/src/shared/Util/RunRules.lua" || fail "implausibly fast trials are not rejected"
...
grep -q 'Practice requires a separately authorized chapter selection' "$ROOT_DIR/src/server/Services/ObbyService.lua" || fail "generic mode remote can enter practice"
...
grep -q 'grid.CellSize = UDim2.fromOffset(64, Theme.MinimumTouchSize)' "$ROOT_DIR/src/client/Controllers/UIController.lua" || fail "practice buttons miss touch target height"
...
grep -q 'Accessibility_reduceFlashes' "$ROOT_DIR/src/client/Controllers/EffectsController.lua" || fail "finale flash reduction missing"
grep -q 'not reducedMotion and not reduceFlashes' "$ROOT_DIR/src/client/Controllers/EffectsController.lua" || fail "finale effects ignore flash reduction"
...
grep -q 'workspace:GetPartsInPart(part, mechanicOverlapParams)' "$ROOT_DIR/src/server/Services/ObbyService.lua" || fail "bounded mechanics use legacy contact polling"
grep -q 'mechanicOverlapParams.MaxParts = 64' "$ROOT_DIR/src/server/Services/ObbyService.lua" || fail "mechanics overlap query is unbounded"
...
grep -q 'incorrect stage ownership' "$ROOT_DIR/src/server/WorldGen/WorldValidator.lua" || fail "collectible stage ownership validation missing"
...
grep -q 'reachedRouteEnd' "$ROOT_DIR/src/server/Services/ObbyService.lua" || fail "cart route boundary recovery missing"
grep -q 'stageIndex <= totalStages' "$ROOT_DIR/src/shared/Util/RunRules.lua" || fail "run stage bounds missing"
...
grep -q 'self:getProfile(player).highestChapter < targetStage' "$ROOT_DIR/src/server/Services/CheckpointService.lua" || fail "practice checkpoint helper lacks unlock guard"
...
test "$(grep -c 'self.totalKeys = countOwnedKeyTags(self.world.model)' "$ROOT_DIR/src/server/Services/ObbyService.lua")" -ge 2 || fail "key total is initialized too late"
...
grep -q 'shutdown save window expired' "$ROOT_DIR/src/server/Services/CheckpointService.lua" || fail "shutdown save deadline missing"
grep -q 'local remaining = #players' "$ROOT_DIR/src/server/Services/CheckpointService.lua" || fail "shutdown saves are not concurrent"
