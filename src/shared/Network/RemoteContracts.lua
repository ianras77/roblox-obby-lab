...
local RemoteContracts = {
  Progress = {
    ...
    payload = "{ stage: number, total: number, mode: string, ready: boolean, highestChapter: number?, chapterId: string?, chapterName: string?, mechanic: string?, flavor: string?, initialized: boolean?, keys: number?, totalKeys: number?, collectedKeys: table?, settings: table?, runStarted: boolean?, elapsedMs: number? }",
  },
  Keys = {
    name = "KeyProgress",
    direction = "server_to_client",
